...
pub trait Tracer: Send + Sync {
    ...
    fn named_round_begins(&mut self, round_name: &'static str) {
        self.trace_event(Event::RoundBegins { name: Some(round_name) })
    }
...
mod requires_std {
    ...
    impl PerfProfiler {
        ...
        pub fn new() -> Self {
            Self {
                ...
                ongoing_stage: None,
                protocol_began: None,
                report: PerfReport { setup: Duration::ZERO, setup_stages: vec![], rounds: vec![], display_io: true },
                error: None,
            }
        }
            ...
            match event {
                ...
                Event::Stage { name } => {
                    ...
                    let stage_i = match stage_i {
                        Some(i) => i,
                        None => {
                            stages.push(StageDuration { name, duration: Duration::ZERO });
                            stages.len() - 1
                        },
                    };
        ...
        fn last_round_mut(&mut self) -> Result<&mut RoundDuration, ProfileError> {
            let last_round = self.report.rounds.last_mut().ok_or(ErrorReason::RoundNeverBegan)?;
            Ok(last_round)
        }
    ...
    impl fmt::Display for PerfReport {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            let total_computation = self.setup + self.rounds.iter().map(|r| r.computation).sum::<Duration>();
            let total_send =
                if self.display_io { self.rounds.iter().map(|r| r.sending).sum::<Duration>() } else { Duration::ZERO };
            ...
            if self.display_io {
                writeln!(f, "    - Computation: {total_computation:.2?} ({})", percent(total_computation, total))?;
                writeln!(f, "    - I/O: {total_io:.2?} ({})", percent(total_io, total))?;
            ...
            for (i, round) in self.rounds.iter().enumerate() {
                Self::fmt_round(
                    ...
                    &round.stages,
                    round.computation,
                    if self.display_io { Some((round.sending, round.receiving)) } else { None },
                )?;
            }
    ...
    impl PerfReport {
        fn fmt_round(
            ...
            if let Some((sending, receiving)) = io {
                ...
                writeln!(f, "    - I/O: {:.2?} ({})", total_io, percent(total_io, total_duration))?;
        ...
        fn fmt_stages(f: &mut fmt::Formatter<'_>, total: Duration, stages: &[StageDuration]) -> fmt::Result {
            for stage in stages {
                writeln!(f, "    - {}: {:.2?} ({})", stage.name, stage.duration, percent(stage.duration, total),)?;
            }
            Ok(())
        }
    }
